...
import { charAbilityMapsTable as localTable } from '../common/db/schema/local/char_ability_maps.table';
import { charAbilityMapsTable as remoteTable } from '../common/db/schema/remote/char_ability_maps.table';

/**
 * @description DB row → 인물-어빌리티 매핑 VO.
 * @param row DB 결과 한 행
 */
...
export const CharAbilityMapMapper = {
  ...
  /**
   * @description 단건 조회 (charNo + abilityNo + abilityType).
   * @param charNo 인물 번호
   * @param abilityNo 어빌리티 번호
   * @param abilityType 어빌리티 타입 (GLOBAL | PROJECT)
   */
