 ...
 */
export function getVoSchemaForUrl(url: string | undefined): z.ZodType | null {
  if (!url) return null;
...
function isResponseEnvelope(
  obj: unknown
): obj is { data?: unknown; error?: boolean; code?: unknown; message?: string } {
...
/**
 * ResponseType/ListResponseType의 data(또는 data.list 항목)를 스키마로 parse해
 * 없는 필드를 null로 채운 객체로 교체.
 * parse 실패 시 원본 body 그대로 반환.
 */
export function normalizeResponseData(body: unknown, schema: z.ZodType): unknown {
  ...
  try {
    if (hasList(data)) {
      ...
      return { ...body, data: { ...data, list, }, };
    }
    if (typeof data === 'object' && data !== null) {
      return { ...body, data: schema.parse(data), };
    }
  }
  catch {
    // parse 실패 시 원본 유지
