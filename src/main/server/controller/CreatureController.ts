...
/**
 * 생물/종족 API 컨트롤러.
 * /creatures 에 마운트. prjNo는 쿼리(GET/PATCH/DELETE) 또는 바디(POST)로 전달.
 * GET/POST /creatures, GET/PATCH/DELETE /creatures/:creatureNo.
 */
...
creatures.patch('/:creatureNo', async (context) => {
  ...
  const result = await CreatureService.update(prjNo, creatureNo, parsed.data ?? {} as Partial<CreatureVo>);
