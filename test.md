...
> **File:** `PLAN_AND_RESULT_PROTOCOL.md`
...
### 📁 Directory Structure
The agent must automatically create these directories relative to the workspace root if they do not exist. **플랜과 결과는 같은 폴더에 생성**하며, **플랜 폴더는 PRD 안에 둡니다.**
```
PRD/
└── plans/
    └── YYYY-MM-DD/
...
```
*별도 results/ 폴더는 사용하지 않습니다. RESULT 파일은 해당 일자의 plans 폴더에 PLAN과 함께 둡니다.*
...
1.  **Verify Result**.
2.  **Create Result File** (플랜과 같은 폴더에 생성):
    ...
    *   **Template**:
        ...
        ...
        ```
3.  **Final Report**: Notify the user of completion.
